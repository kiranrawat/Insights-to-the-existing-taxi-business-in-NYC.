...
def main(input_data):
    ...
    weekly_summary = tripdata.groupBy('medallion', 'hack_license', sf.weekofyear('pickup_datetime').alias('p_week')).agg(sf.sum('trip_distance').alias('sum_dist'),
                                                                                                                          sf.sum('total_amount').alias('sum_amount'),
                                                                                                                          sf.sum('trip_time_in_secs').alias('sum_trip'),
                                                                                                                          sf.sum('passenger_count').alias('passenger_count'))
    weekly_summary.write.format('json').mode('overwrite').save('analysis_result/driver_weekly')

...
def main(input_data, input_fare, output):
    ...
    monthly_summary = joined_df.groupBy('medallion', 'hack_license', sf.month('pickup_datetime').alias('p_month')).agg(sf.sum('trip_distance').alias('sum_dist'), sf.sum('total_amount').alias('sum_amount'), sf.sum('trip_time_in_secs').alias('sum_trip'), sf.sum('passenger_count').alias('passenger_count'))
    monthly_summary.write.format('json').save(output)
